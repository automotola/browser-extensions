...
bool FrameProxy::Is64BitProcess(DWORD processId)
{
  ...
  if (fnIsWow64Process) {
    DWORD access = version.dwMajorVersion >= 6 ? PROCESS_QUERY_LIMITED_INFORMATION : PROCESS_QUERY_INFORMATION;
    HANDLE process = ::OpenProcess(access, false, processId);
    if (process) {
      if (!fnIsWow64Process(process, &isWow64)) {
        DWORD error = ::GetLastError();
        logger->error(L"FrameProxy::Is64BitProcess IsWow64Process failed -> " + boost::lexical_cast<wstring>(error));
        isWow64 = FALSE;
      }
      ::CloseHandle(process);
    }
      ...
      DWORD error = ::GetLastError();
      logger->error(L"FrameProxy::Is64BitProcess failed to open process -> " + boost::lexical_cast<wstring>(error));
    }
  }
  else {
  ...
  logger->debug(L"FrameProxy::Is64BitProcess isWow64 -> " + boost::lexical_cast<wstring>(isWow64));
...
bool FrameProxy::InjectDLL(HINSTANCE instance, DWORD processId)
{
  ...
  logger->debug(L"FrameProxy::InjectDLL spawning process -> " + path.wstring());

  if (!::CreateProcess(path.wstring().c_str(), params, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &startupInfo, &processInfo)) {
  ...
  if (exitCode != 0) {
    logger->error(L"FrameProxy::InjectDLL spawned process failed -> " + boost::lexical_cast<wstring>(exitCode));
    return false;
  }
...
{
  logger->debug(L"FrameProxy::MessageHandlerListener -> " + boost::lexical_cast<wstring>(param));
  ...
  while (channel->Read(buffer, Channel::SECTION_SIZE)) {
    ...
    logger->debug(L"FrameProxy::MessageHandlerListener type -> " + boost::lexical_cast<wstring>(type));
    ...
    case ForwardedMessage::COMMAND_TYPE: {
      ...
      LRESULT result;
      proxy->WndProcTarget(&result, message->msg, message->wparam, message->lparam);
      break;
    }
    ...
    default: {
      logger->debug(L"FrameProxy::MessageHandlerListener unknown type -> " + boost::lexical_cast<wstring>(type));
      ForwardedMessage *message = (ForwardedMessage*)buffer;
      FrameProxy       *proxy = (FrameProxy*)message->proxy;
      LRESULT result;
      proxy->WndProcTarget(&result, message->msg, message->wparam, message->lparam);
 ...
 *
 */
bool FrameProxy::WndProcTarget(LRESULT *lresult, UINT message, WPARAM wparam, LPARAM lparam)
{
  ...
  switch (message) {
  case WM_COMMAND:
    logger->debug(L"FrameProxy::WndProcTarget WM_COMMAND -> " + boost::lexical_cast<wstring>(wparam)+L" -> " + boost::lexical_cast<wstring>(lparam));
    break;
  case 0xdeadbeef:
    logger->debug(L"FrameProxy::WndProcTarget Got some reply");
    break;
  default:
    logger->debug(L"unknown message -> " + boost::lexical_cast<wstring>(message));
    break;
  };
