...
{
public:
  FrameProxy(const wstring& uuid, HINSTANCE instance, HWND toolbar, HWND target, const wstring& title = L"", const wstring& icon = L"");
  virtual ~FrameProxy();
  ...
  {
    logger->debug(L"FrameProxy::Read -> " + boost::lexical_cast<wstring>(dataSize));
  ...
  {
    logger->debug(L"FrameProxy::Write -> " + boost::lexical_cast<wstring>(dataSize));
    command.processId = ::GetCurrentProcessId();
    command.proxy = (INT_PTRX)this;
    return m_commandChannel->Write(&command, dataSize, response);
  }

  virtual bool WndProcTarget(LRESULT *lresult, UINT message, WPARAM wparam, LPARAM lparam);
