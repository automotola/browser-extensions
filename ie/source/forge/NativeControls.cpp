 ...
 * Construction
 */
CNativeControls::CNativeControls() : m_frame(NULL), m_frameProxy(NULL), instanceCounter(1)
{
...
{
  logger->debug(L"CNativecontrols::unload -> " + wstring(uuid) + L" -> " + boost::lexical_cast<wstring>(instanceId));
...
STDMETHODIMP CNativeControls::popup_visible(BSTR uuid, BOOL isVisible, POINT point)
{
  ...
  PopupWindow::pointer popup = m_popupWindows[uuid];
  if (!popup) {
    logger->debug(L"CNativeControls::popup_visible has no popup -> " + wstring(uuid));
    return E_FAIL;
  }

  // get screen metrics
  DWORD screenWidth = ::GetSystemMetrics(SM_CXSCREEN);
  popup->alignment = point.x > (LONG)(screenWidth / 2)
    ? PopupWindow::left
    : PopupWindow::right;
  ...
  popup->MoveWindow(origin, point.y, width, height);
 ...
 * @param uuid
 */
STDMETHODIMP CNativeControls::button_setIcon(BSTR _uuid, BSTR _url, IDispatch *success, IDispatch *error)
{
  ...
  // check if it exists
  if (!bfs::exists(path)) {
    logger->debug(L"CNativeControls::button_setIcon invalid path for icon -> " + path.wstring());
    return CComDispatchDriver(error).Invoke1((DISPID)0, &CComVariant(L"{ 'message' : 'invalid path for icon' }"));
  }
...
STDMETHODIMP CNativeControls::button_onClicked(BSTR uuid, IDispatch *callback)
{
  ...
  button_listeners[wstring(uuid)] = callbackex;
 ...
 */
STDMETHODIMP CNativeControls::Invoke(DISPID dispid, REFIID riid, LCID lcid, WORD flags, DISPPARAMS *params, VARIANT *result, EXCEPINFO *excepinfo, UINT *arg) 
{
  ...
  return IDispatchImpl<INativeControls, &IID_INativeControls, &LIBID_ForgeLib, 1, 0>::Invoke(dispid, riid, lcid, flags, params, result, excepinfo, arg);
}
